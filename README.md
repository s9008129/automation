# RPA-Cowork — 內部網路自動化協作工作流

> 一個為內部網路場景設計的離線優先 RPA 工作流：透過 AI 協作，讓非技術同仁也能打造並執行專屬的自動化腳本。
...
本專案打通了「**蒐集 → AI 協作 → 執行**」的完整工作鏈，讓你：

1. 在內網蒐集高品質頁面素材（ARIA 快照、截圖、錄製）
2. 把素材帶到外部 AI 環境分析
3. 讓 AI 協助生成可用的 TypeScript 自動化腳本
4. 把腳本帶回內網，透過 `run-task.ps1` 一鍵執行
5. 遇到錯誤時，把錯誤訊息交給 AI 持續修正

> 目標是**讓非技術同仁也能打造自己的專屬自動化腳本**，而不只是蒐集素材。
...
┌──────────────────┐          ┌─────────────────────────────────┐          ┌──────────────────────┐
│ prepare bundle   │          │ ① install.ps1                   │          │ ① 讀取蒐集成果        │
│ 複製整個資料夾    │  ──────▶  │ ② launch-edge.ps1               │  ──────▶  │ ② AI 分析頁面結構     │
│                  │          │ ③ collect.ps1  ─素材─▶ materials\│          │ ③ 生成任務腳本        │
│                  │  ◀──────  │ ④ run-task.ps1 ◀─腳本─────────  │  ◀──────  │ ④ 修正錯誤訊息       │
└──────────────────┘          └─────────────────────────────────┘          └──────────────────────┘
```
...
collect.ps1              →  Claude / GPT           →  run-task.ps1
ARIA 快照                    分析頁面結構               執行 .ts 腳本
截圖、錄製                   生成任務腳本               中文錯誤提示
HTML 原始碼                  分析錯誤、修正             查看日誌、持續改進
```
...
> AI 協作工作流與任務腳本說明，請看 [`docs/AI協作工作流.md`](docs/AI協作工作流.md)。
...
### [`docs/AI協作工作流.md`](docs/AI協作工作流.md)
說明如何透過 AI 協作將素材轉化為自動化腳本，以及如何執行與持續修正。

### [`docs/任務腳本開發規範.md`](docs/任務腳本開發規範.md)
說明任務腳本的技術規範、命名慣例與共用 Helper 使用方式。

### [`docs/執行任務指南.md`](docs/執行任務指南.md)
說明如何使用 `run-task.ps1` 執行任務腳本，以及常見錯誤排除。

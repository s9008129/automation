...
簡短說明
Selenium 可視為一位資深且通用的司機，能駕馭多種瀏覽器；Playwright 則像新一代的自動駕駛系統，內建現代化感測與控制機制，在複雜或受限環境下更穩定可靠。

主要差異（重點簡述）
- API 現代且一致：更直覺，減少同步與等待問題。
- 穩定性高：內建自動等待與重試，降低脆弱性。
- ARIA / 無障礙支援：能直接擷取頁面語意結構（對素材蒐集非常有用）。
- 可附加到已開啟的 Chrome（connectOverCDP）：能使用使用者已登入的瀏覽器會話而不需重啟或登入。
- 內建截圖、網路攔截與互動錄製工具：方便蒐集完整素材。
- 社群快速成長，適合現代化自動化需求。

為何 Playwright 適合本專案（3 點）
1. connectOverCDP：可以安全地附加到使用者已登入的 Chrome，支持在不關閉使用者瀏覽器的前提下蒐集素材（實作時仍應避免在不確定情況下強制關閉使用者的 Chrome）。
2. ARIA-first 流程支援：能直接抓取 accessibility tree（ARIA），提供高價值的語意素材給後續 AI 分析。
3. 離線與穩定性：在內網、預先安裝套件的情況下仍能穩定執行，減少因網路或外部服務導致的中斷。

對 Selenium 使用者的務實過渡建議
1. 先挑 1–2 個最關鍵的流程做 Proof-of-Concept（POC），以 Playwright 重現並比對輸出與穩定性。
2. 用 Playwright 的自動等待與 ARIA 選擇器取代原本的顯式等待／sleep，逐步移除容易造成脆弱性的同步邏輯。
3. 在 connectOverCDP 模式下測試附加既有 Chrome，確認不會關閉使用者瀏覽器並更新錯誤處理（避免在附加模式下無條件呼叫 browser.close()）。
...
User:
目標：自動登入 X 系統 → 進入「資安聯防監控月報」 → 下載前一個月的 PDF（存到 ./output）。
...
User:
目標：自動登入 X 系統（使用 process.env.NCERT_USERNAME / NCERT_PASSWORD），導航到「資安訊息公告」→展開子選單→點選「資安聯防監控月報」，下載第二列 PDF（STRICT_SECOND_ROW 可設定）。
上下文：

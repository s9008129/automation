...
蒐集素材並請 AI 生成腳本後，用 `run-task.ps1` 執行：
...
# 帶參數執行
.\run-task.ps1 src\vst-資料匯出.ts --dry-run
```
...
詳細說明請看 [`docs/執行任務指南.md`](執行任務指南.md)。

---

## 9. 一句話記住

**一般使用者只要記住 `install.ps1 -> launch-chrome.ps1 -> collect.ps1`；如果現場指定用 Edge，就改成 `install.ps1 -> launch-edge.ps1 -> collect.ps1 --browser edge`。蒐集完素材、AI 幫你生成腳本後，用 `.\run-task.ps1 src\腳本名稱.ts` 執行。**

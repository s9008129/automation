...
### 13.7 高品質 Prompt 範本（Template）

為讓 LLM 生成穩定、可執行且符合專案約束的 Playwright TypeScript 腳本，請使用下列以「第一性原則」為框架的 Prompt 模板（Goal、Context、Inputs、Constraints、Validation、Interaction）。填寫時請以繁體中文，並務必把 ARIA 快照、recording 與 metadata 的內容一併貼上。

Prompt 結構說明（每一欄位均為必填或說明）：
- 目標 (Goal)：一句話明確描述你要自動化的結果（例如：「自動登入 X 系統並下載上個月報表 PDF」）
- 上下文 (Context)：執行環境（離線 / Windows 11 + PowerShell / Playwright 版本 / CDP port），是否已在 Chrome 完成登入等
- 可用素材 (Available materials)：列出實際檔名（aria-snapshots/*.txt、recordings/*.ts、screenshots/*.png、metadata.json）並在下方貼上對應內容
- 限制 (Constraints)：明確列出非談判條件（例如：離線、不使用外部網路、不呼叫 browser.close()、必須使用 connectOverCDP）
- 安全/隱私 (Security)：不能在腳本中硬編碼密碼或 token，所有敏感值必須使用 process.env; 指示 LLM 在回覆中提醒要檢查 .env
- 執行限制 (Execution constraints)：不得關閉使用者 Chrome、支援 CDP_PORT 環境變數、優先使用 ARIA 語意選擇器
- 輸出要求 (Output)：回覆必須包含完整可執行的 TypeScript (.ts)、簡短執行說明、需要的 .env 變數清單、以及用於驗證的最小測試腳本
- 驗證步驟 (Validation)：明確要求 LLM 同時提供驗證步驟，例如：`npx tsc --noEmit`、執行 minimal-test.ts、在有 Chrome CDP 的實機上跑一次並擷取截圖與日誌
- 最小範例輸入 (Minimal sample)：提供 1~2 個最小必要的 ARIA 片段與 recording 片段，讓 LLM 有足夠上下文產出最小可執行腳本

範本 — User Prompt（可直接貼入 LLM 對話）：

````markdown
## 目標 (Goal)
自動登入「XXX 系統」，導航到「報表頁」，下載「前一個月」的 PDF，並把檔案存到 ./output/
...
- aria-snapshots/01-login-page.txt  （貼上完整內容）
- aria-snapshots/02-reports-list.txt （貼上完整內容）
- recordings/login-flow.ts           （貼上完整內容）
- metadata.json                      （貼上完整內容）
...
- 若目標連結位於隱藏下拉，請提供 Robust Reveal Strategy（hover + dispatchEvent + mouse.move + focus），並提供 fallback 為直接導航到已知列表頁
- 檔名需經 safeFileName() 淨化並保證 .pdf 副檔名
- 若表格定位需使用第二列，請支援 STRICT_SECOND_ROW 環境旗標（true/false）
...
## 驗證步驟 (Validation)
1. 執行 `npx tsc --noEmit`（應零錯誤）
2. 執行 `npx tsx minimal-test.ts` 確認 CDP 可連接
3. 在實機跑一次腳本並上傳執行時的 logs 與完成截圖供驗證
...
A) 精簡版（快速驗證）
````markdown
System:（請先貼 13.2 節的 System Prompt）

User:
目標：登入 X 系統並下載前一個月的報表 PDF。
上下文：Windows 11、CDP 9222、Playwright 1.52、Chrome 已登入。
素材：aria-snapshots/02-reports-list.txt（貼上）
限制：離線、不含密碼、使用 process.env。請回覆一個可執行的 TypeScript 腳本與最小驗證步驟。
````

B) 標準版（推薦）
````markdown
System:（貼上 13.2 節 System Prompt）
...
（新增提示）：建議把 System Prompt (13.2) 設為 LLM 的 System Message，然後以上述 User Prompt 模板貼上素材內容；這可以最大化生成腳本的穩定性與可驗證性。
...
### 14.6 改良 Debug Prompt 範本（Template）

說明：當你要請 LLM 幫忙除錯 AI 生成或手動修改後的 Playwright 腳本時，請依下列清單準備資料並使用範本；範本強制要求 LLM 在資料不足時提出不超過 5 個可選澄清問題，並在使用者回答後才輸出可套用的程式碼修改（unified diff 或完整檔案）。

使用者需提供的資料（必填項）與檔名建議：
- ARIA 快照（出錯頁面）
  - 檔名範例：materials\aria-snapshots\03-report-list.txt
  - 取得方式：在已登入的 Chrome 中用本工具擷取（參見第 5 節）或貼上該檔案內容
- Hover/Reveal 狀態截圖（若下拉/懸浮有顯示差異）
  - 檔名範例：materials\screenshots\03-report-list-hover.png
  - 取得方式：在 Chrome 中 reproduce 該畫面並用系統截圖工具或 DevTools Capture node screenshot
- 錄製檔（recording）
  - 檔名範例：materials\recordings\login-flow.ts
  - 說明：貼上或附上 recording 內容，方便 LLM 理解互動順序
- PowerShell 錯誤輸出與完整 Stack Trace
  - 檔名範例：logs\error-20260212-170000.txt（或直接貼上錯誤段落）
  - 取得方式：在 PowerShell 中全選輸出（Ctrl+A → Ctrl+C），貼到記事本
- logs 片段（包含 timestamp）
  - 提供與錯誤發生時間相近的日誌（建議多給 10s 前後）
- 相關環境旗標（如 STRICT_SECOND_ROW、CDP_PORT）與 .env 是否已正確設定（不要貼密碼）

檔案命名與貼上規則（能讓 LLM 自動解析）：
- 每個檔案在貼上前以標題註明檔案名稱，例如：
...
互動式問答流程規範（LLM 必須遵守）：
1. 若收到的資料不完整或不明確，LLM **不得直接修改程式碼**，必須先列出「不超過 5 個」且互為選擇的澄清問題（prefer multiple-choice or yes/no where possible）。
2. 每個問題請提供 2~4 個選項（若合適請把最可能的選項放第一個並標註 Recommended）。
3. 使用者回覆後，LLM 以使用者回覆為唯一依據進行修正建議，並輸出：
   - Root cause（簡短清楚）
   - 一套逐步修正步驟（步驟化）
   - 可直接貼上的修正程式碼（unified diff 或完整檔案）
   - 重新驗證步驟（如何在本專案執行並確認問題已解）
4. 所有修改內容需註明風險（例如：「此修改在某些舊版 Playwright 可能不起作用」）與 fallback 建議

Debug Prompt 範本（可直接貼給 LLM）

````markdown
## 問題摘要
【用白話說明發生的問題】（例如：「腳本找不到下拉選單內的報表連結」）

## 已附資料
- ARIA: materials/aria-snapshots/03-report-list.txt （已貼上）
- 截圖: materials/screenshots/03-report-list-hover.png
- 錄製檔: materials/recordings/login-flow.ts
- PowerShell 錯誤輸出: logs/error-20260212-170000.txt（已貼上錯誤與 stack）
- 環境旗標: STRICT_SECOND_ROW=true, CDP_PORT=9222

## 我希望你做的事
1. 若資料不足請先提出不超過 5 個可選澄清問題（不要直接假設）
2. 基於完整資料，請提供 Root Cause、步驟化修正建議、以及可直接套用的修正程式碼（unified diff 或完整檔案）
3. 提供最少 2 個驗證步驟與 fallback 措施
````

下方提供 3 個實際 Debug Prompt 範例（供複製貼上）：

範例 1 — 無法 hover 揭露下拉但可直接導航列表頁
````markdown
問題摘要：腳本試圖透過 hover 展開「資安訊息公告」下拉選單並點選「資安聯防監控月報」，但 playright 的 hover 與 click 都失敗（TimeoutError: waiting for selector）。直接導航到列表頁（Post2/list.do）可以看到報表。
...
請先檢查資料是否足夠，若不足請提出最多 5 個選擇式問題（例如：是否能提供 hover 的截圖 / 錄製的 selector / 是否有 iframe）
...
請回應：
1) 若資料不足，先提出不超過 5 個選擇題（例如：是否能貼上完整 table HTML / 是否包含可辨識的日期欄 / 是否能提供錄製檔中該 table 的互動片段）
2) 基於資料，建議最穩健的 row 選取邏輯（例如：識別具有日期欄且日期為前一個月的 row；或使用檔名匹配正則；或以 aria role 與 td 數量判斷 data row），並給出可套用的程式碼修改
...
（備註）本範本把交互式問答規範直接寫入 Prompt，能最大化避免 LLM 在資料不足時做出錯誤假設，並保證使用者後續可套用的修正具有可驗證性與最小副作用。

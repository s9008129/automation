...
- **選 [3]**：結束蒐集，工具會儲存 metadata.json 和 summary-report.md
...
#### 頁面 2：【頁面名稱，例如：報表列表頁】
```
【貼上 aria-snapshots/02-reports-list.txt 的完整內容】
```
（依此類推，每個頁面一個區塊）
...
素材：aria-snapshots/02-reports-list.txt（貼上）
...
素材：
- aria-snapshots/01-login-page.txt（貼上）
- aria-snapshots/02-reports-list.txt（貼上）
- recordings/login-flow.ts（貼上）
...
- CDP_PORT: 9222
可用素材（請一一貼上內容）：
- aria-snapshots/01-login-page.txt
- aria-snapshots/02-menu.txt（包含主選單與子選單）
- aria-snapshots/03-report-table.txt
- recordings/login-and-open-report.ts
已知限制/邊界條件：
...
| 資料項目 | 檔名範例 | 如何取得 | 必要性 |
|---------|---------|---------|--------|
| ARIA 快照（出錯頁面） | `materials\aria-snapshots\03-report-list.txt` | 用本工具擷取（參見第 5 節）或貼上檔案內容 | ✅ 必要 |
...
| 錄製檔（recording） | `materials\recordings\login-flow.ts` | 貼上 recording 內容 | ⭐ 建議 |
| Hover/Reveal 狀態截圖 | `materials\screenshots\03-hover.png` | 在 Chrome 中重現該畫面並截圖 | ⭐ 建議 |
...
```text
--- FILE: materials/aria-snapshots/03-report-list.txt ---
(貼上 aria 內容)
--- END FILE ---
```
...
### 相關的 ARIA 快照（出錯頁面）
```
【貼上報錯時所在頁面的 ARIA 快照內容，例如 aria-snapshots/03-report-list.txt】
```
...
附檔：
--- FILE: materials/aria-snapshots/02-menu.txt ---
(貼上 menu aria 內容)
--- END FILE ---
...
附檔：
--- FILE: materials/aria-snapshots/03-report-table.txt ---
(貼上 table aria 內容)
--- END FILE ---

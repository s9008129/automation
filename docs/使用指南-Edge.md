...
`new-task.ps1` 會問你「系統代碼」和「任務名稱」，然後把 `templates\task-template.ts` 複製成一份新的 `src\系統代碼-任務名稱.ts`。

你可以把它理解成：

- `templates\task-template.ts` = 工具內建的原始範本
- `new-task.ps1` = 幫你把範本變成這次任務專用骨架的入口
- `src\你的腳本.ts` = 你這次真正要交給 AI、最後也會拿來執行的檔案

這份「骨架」不是完整腳本，但它已經先幫你排好本專案固定格式，例如：
...
所以，一般使用者**不用先看懂 `task-template.ts` 每一行**。你只要知道：`new-task.ps1` 是先幫你開一份「格式已經對的答案卷」，AI 要做的是把內容補進去，不是另外重做一個新專案。

你真正會操作的順序是：

1. `.\new-task.ps1`
2. 拿到 `src\你的腳本.ts`
3. 把 `src\你的腳本.ts` 和同一次任務附件交給 AI
4. 用 AI 回傳內容覆蓋 `src\你的腳本.ts`
5. 執行 `.\run-task.ps1 src\你的腳本.ts`

### 如果我沒有上傳整個專案，AI 怎麼知道本專案框架？

先講結論：**AI 不會自動看到你的整個 repo。**

Prompt 裡寫「請遵守本專案框架」，只是提醒 AI 要守規則，不代表 AI 已經真的看過這個專案。所以，要讓 AI 產生可直接執行的腳本，至少要讓它同時看到兩種資訊：

1. **框架線索**：這個專案的腳本放哪裡、怎麼 import、怎麼執行、哪些規則不能違反
2. **任務線索**：這次頁面流程長什麼樣、按鈕文字是什麼、成功畫面長什麼樣

對這個專案來說，`new-task.ps1` 產生的 `src\腳本骨架.ts` 本身，就是最重要的框架線索。因為它已經把 `runTaskEntry(...)`、`src\lib\*`、Edge 預設方式都先放進去了。
...
你不需要一開始就把整個 repo 都丟給 AI。先照下面兩層準備就好。
...
- `recordings\`
- `metadata.json`
- `src\腳本骨架.ts`（一定要先用 `new-task.ps1` 建好）
...
如果 AI 常常寫成一般 Playwright 範例、自己加 `npm install`、沒用 `runTaskEntry(...)`，通常就是框架線索不夠，這時再補上面這 4 份即可。

> ⚠️ 不要上傳 `.env`。裡面可能有帳號、密碼或 token。

### 框架前言（建議貼在 Prompt 前面）

如果你**沒有上傳整個 repo**，建議先貼下面這段，再貼後面的生成 / 除錯 Prompt。

````text
先說明：你看不到我的整個 repo，請只根據我上傳的骨架腳本、附件與以下規則工作。這不是要你另外建立一個新的 Playwright 專案，而是要你補完既有 RPA-Cowork 框架內的 `src\任務腳本.ts`。

固定規則：
- 我已先用 `new-task.ps1` 建好骨架，最後會用 `.\run-task.ps1 src\腳本名稱.ts` 執行
- 腳本必須留在 `src\`
- 腳本入口固定使用 `runTaskEntry(...)`
- 優先重用 `./lib/env.js`、`./lib/logger.js`、`./lib/browser.js`、`./lib/security.js`
- 預設使用 Edge：`launchTaskBrowser({ channel: 'msedge', headless: false })`
- 不可要求 `npm install`、`npx`、`npm run` 或額外下載套件
- 敏感資料必須從 `.env` 讀取，不可硬編碼，也不可 `import dotenv`
- 如果附件不足，請先列出缺少資訊，不要自行假設
````

### Edge 版生成腳本 Prompt（空白版）

如果你沒有上傳整個 repo，建議先貼上面的「框架前言」，再貼下面這段。
...
如果你沒有上傳整個 repo，建議先貼上面的「框架前言」，再上傳附件（含失敗的腳本、錯誤日誌）並貼上下面這段。

...
**建立日期**: 2026-02-09
**最後更新**: 2026-03-10（v1.3.1 — 新增 `npm run collect` 任務子資料夾輸出規格）
**狀態**: v1.3.1 — 規格已更新（核心功能現況：ARIA 快照 ✅ 截圖 ✅ Codegen 錄製 ✅ 互動模式 ✅ 優雅關閉 ✅ 錄製密碼清理 ✅ Pre-commit Hook ✅ ARIA-first 工作流 ✅ T-03 停用 ✅）
...
**步驟**：
1. 使用互動模式（`npm run collect`）逐頁擷取 ARIA 快照與截圖；該次任務輸出統一寫入 `materials\<YYYYMMDDhhmmss_錄製名稱>\`
...
- **FR-008**: 系統 MUST 支援互動模式（一步一步引導蒐集），且每次執行 `npm run collect` 時 MUST 以新的任務子資料夾承載本次蒐集成果
- **FR-009**: 系統 MUST 支援自動模式（依設定檔批次蒐集）
- **FR-010**: 系統 MUST 支援快照模式（快速擷取當前頁面）
- **FR-011**: 系統 MUST 支援錄製模式（直接啟動 Codegen），且錄製輸出 MUST 寫入本次 `npm run collect` 的任務子資料夾
...
- **FR-012**: 系統 MUST 保留 `materials/` 作為輸出根目錄，並在每次執行 `npm run collect` 時建立 `materials\<YYYYMMDDhhmmss_錄製名稱>\` 任務子資料夾
...
- **FR-045**: `npm run collect` 任務子資料夾名稱中的時間戳 MUST 使用 Asia/Taipei (UTC+8)
...
| collectInteractive() | 互動模式蒐集 | 採用「頁面蒐集 → 流程錄製」兩階段選單，最多 100 頁，正常流程不需要用 Ctrl+C 切換階段；整次 `npm run collect` 的輸出集中於同一個 `materials\<YYYYMMDDhhmmss_錄製名稱>\` 任務子資料夾 |
...
1. **Given** Chrome 已以 Debug 模式啟動並登入內部網站，**When** 執行 `npm run collect` 並選擇互動模式，**Then** 系統連接到 Chrome、為本次任務準備 `materials\<YYYYMMDDhhmmss_錄製名稱>\` 任務子資料夾，並顯示兩階段工作說明與當前頁面資訊
...
- 時區：所有 timestamp 與 `npm run collect` 任務子資料夾命名必須使用 Asia/Taipei（UTC+8）。

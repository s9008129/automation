...
**建立日期**: 2026-02-09
**最後更新**: 2026-03-12（v1.4.0 — 新增 PowerShell 一般使用者入口與完整離線包流程）
**狀態**: v1.4.0 — 規格已更新（核心功能現況：ARIA 快照 ✅ 截圖 ✅ Codegen 錄製 ✅ 互動模式 ✅ 優雅關閉 ✅ 錄製密碼清理 ✅ Pre-commit Hook ✅ ARIA-first 工作流 ✅ 離線包流程 ✅）
...
- **FR-023**: scripts\prepare-offline-bundle.ps1 MUST 供技術人員準備完整離線包，包含專案內建 runtime、node_modules 與 Playwright Chromium
- **FR-042**: 技術人員為 Windows 內網使用者準備離線環境時，MUST 先補齊 node_modules 與 Playwright 瀏覽器，並建議使用 scripts\prepare-offline-bundle.ps1 產出完整離線包
...
1. **Given** Chrome 已以 Debug 模式啟動並登入內部網站，**When** 執行 `.\collect.ps1` 並選擇互動模式，**Then** 系統連接到 Chrome、為本次任務準備 `materials\<YYYYMMDDhhmmss_錄製名稱>\` 任務子資料夾，並顯示兩階段工作說明與當前頁面資訊
2. **Given** 系統已連接到 Chrome，**When** 使用者輸入頁面名稱和描述，**Then** 系統自動擷取 ARIA 快照和截圖並儲存到本次任務子資料夾中的 `aria-snapshots\` 與 `screenshots\`

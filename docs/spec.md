...
**步驟**：
1. 使用互動模式（`npm run collect`）逐頁擷取 ARIA 快照與截圖
2. 在頁面蒐集階段的選單中，選擇「切換到錄製階段」
3. 在錄製階段選單中，選擇開始新的錄製
4. 在錄製視窗中操作完整流程，關閉視窗結束錄製
5. 錄製檔自動經過 sanitizeRecording() 清理敏感資訊
...
| startCodegenRecording(flowName, startUrl, instructions) | 啟動 Playwright Codegen 錄製 | Windows 上使用 cmd.exe /d /s /c 包裝；啟動前會明確提示「新開的是獨立錄製視窗」；視窗關閉並生成檔案後會自動呼叫 sanitizeRecording()，並把使用者帶回清楚的後續選單。 |
...
| collectInteractive() | 互動模式蒐集 | 採用「頁面蒐集 → 流程錄製」兩階段選單，最多 100 頁，正常流程不需要用 Ctrl+C 切換階段 |
...
1. **Given** Chrome 已以 Debug 模式啟動並登入內部網站，**When** 執行 `npm run collect` 並選擇互動模式，**Then** 系統連接到 Chrome 並顯示兩階段工作說明與當前頁面資訊
...
5. **Given** 使用者選擇結束蒐集，**When** 使用者在選單中選擇結束，**Then** 系統儲存 metadata.json 和 summary-report.md 並安全斷開連接
...
1. **Given** 在互動模式中蒐集了頁面素材，**When** 使用者切換到錄製階段並選擇開始新的錄製，**Then** 系統提示輸入錄製名稱和起始 URL

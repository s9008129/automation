...
- **FR-001**: 系統 MUST 透過 Playwright `chromium.connectOverCDP()` 連接到使用者已開啟的 Chromium branded browser Debug 模式（Chrome / Edge）
- **FR-002**: 系統 MUST 過濾瀏覽器內部頁面（`chrome://`、`chrome-extension://`、`chrome-untrusted://`、`edge://`、`edge-extension://`、`devtools://`、`about:blank`、`about:srcdoc`），只操作使用者可見的 http/https 頁面
...
- **FR-007**: 系統 MUST 在斷開連接時 NEVER 呼叫 `browser.close()`（保持使用者瀏覽器運行），只將 `this.browser = null`
- **FR-046**: 系統 MUST 支援 `--browser chrome|edge` 與設定檔 `browser` 欄位，且 CLI 參數優先於設定檔

### 2.2 CDP 連線原則（Non-Negotiable）

| 原則 | 說明 |
|------|------|
| **NEVER 強制關閉使用者瀏覽器** | 使用 connectOverCDP 時，斷開連接只做 `this.browser = null`，避免中斷使用者工作 |
| **過濾瀏覽器內部頁面** | `chrome://`、`edge://`、擴充套件頁面、`devtools://`、`about:blank`、`about:srcdoc` 不是使用者頁面 |
| **CDP 預存頁面處理** | `page.url() === ""` 的頁面使用 `CDPSession.send('Runtime.evaluate')` 取得真實 URL，並自動開新分頁導航 | page.url() === '' 的頁面使用 CDPSession.send('Runtime.evaluate') 取得真實 URL，並自動開新分頁導航 |
...
原本設計中，錄製完成後系統會自動解析錄製檔中的 page.goto() URL（透過 extractUrlsFromRecording()），然後逐一導航擷取 ARIA 快照（透過 captureSnapshotsForUrls()）。

**此功能已被停用**，原因如下：

1. **Session 依賴問題**：錄製檔中的 URL 通常指向需要登入才能存取的頁面。自動導航時無法保證已登入狀態，導致擷取到的是登入頁或錯誤頁面。
2. **Codegen 使用獨立瀏覽器**：Playwright Codegen 會啟動自己的瀏覽器視窗；即使主流程附加的是使用者的 Chrome 或 Edge，Codegen 視窗仍是獨立瀏覽器實例，兩者的 session/cookie 不共享。
...
- **FR-008**: 系統 MUST 支援互動模式（一步一步引導蒐集），且每次執行 `.\collect.ps1` 時 MUST 以新的任務子資料夾承載本次蒐集成果
- **FR-009**: 系統 MUST 支援自動模式（依設定檔批次蒐集）
- **FR-010**: 系統 MUST 支援快照模式（快速擷取當前頁面）
- **FR-011**: 系統 MUST 支援錄製模式（直接啟動 Codegen），且錄製輸出 MUST 寫入本次 `.\collect.ps1` 的任務子資料夾
...
- **FR-012**: 系統 MUST 保留 `materials/` 作為輸出根目錄，並在每次執行 `.\collect.ps1` 時建立 `materials\<YYYYMMDDhhmmss_錄製名稱>\` 任務子資料夾
- **FR-013**: 系統 MUST 在該任務子資料夾根目錄輸出 metadata.json（蒐集記錄、頁面統計、錯誤記錄）與 summary-report.md（摘要報告、檔案結構說明）
- **FR-014**: ARIA 快照 MUST 包含頁面 URL、標題、擷取時間、專案名稱等 header 資訊
- **FR-044**: `aria-snapshots\`、`screenshots\`、`recordings\` 等既有輸出目錄 MUST 位於對應任務子資料夾下，不可直接散落於 `materials/` 根目錄

### 2.9 Windows 特殊說明

- **FR-018**: 系統 MUST 在 Windows 上使用 `cmd.exe /d /s /c` 啟動需要 shell 字串的子程序（避免 spawn EINVAL）
...
- **FR-050**: Edge 模式 MUST 視 Microsoft Edge 為系統前置需求，不可把 branded Edge 視為離線包必帶內容
...
- **FR-048**: macOS / Linux MUST 使用 `scripts/launch-edge.sh` 啟動 Edge Debug 模式
...
| Edge 啟動日誌 | logs\launch-edge-YYYYMMDD-HHMMSS.log | launch-edge.ps1 |
| 素材蒐集日誌 | logs\collect-materials-YYYYMMDD-HHMMSS.log | collect-materials.ts |

#### 日誌行格式

- **FR-031**: 格式為 [YYYY-MM-DDTHH:mm:ss+08:00][層級] emoji 訊息
- **FR-045**: `.\collect.ps1` 任務子資料夾名稱中的時間戳 MUST 使用 Asia/Taipei (UTC+8)
- 層級為 INFO / WARN / ERROR / CONTEXT

### 3.4 metadata.json 規範

- **FR-029**: `metadata.json` MUST 包含以下欄位：`projectName`、`description`、`browser`、`browserVersion`、`collectedAt`、`timezone`、`toolVersion`、`platform`、`nodeVersion`、`playwrightVersion`、`logFile`、`totalPages`、`collectedPages`、`recordings`、`errors`
...
在提交或合併程式碼前，建議執行以下審查：
1. 確認錄製檔（`materials\<任務子資料夾>\recordings\*.ts`）中不含明文密碼
...
| extractUrlsFromRecording(filePath) | 從錄製檔提取 page.goto() URL | ⛔ **已停用（T-03 disabled）**：因 session 依賴問題無法可靠運作 |
| captureSnapshotsForUrls(urls, flowName) | 為提取的 URL 自動擷取 ARIA 快照 | ⛔ **已停用（T-03 disabled）**：搭配 extractUrlsFromRecording 使用，一併停用 |
| startCodegenRecording(flowName, startUrl, instructions) | 啟動 Playwright Codegen 錄製 | Windows 上使用 cmd.exe /d /s /c 包裝；啟動前會明確提示「新開的是獨立錄製視窗」；視窗關閉並生成檔案後會自動呼叫 sanitizeRecording()，並將錄製結果寫入本次任務子資料夾的 `recordings\`，再把使用者帶回清楚的後續選單。 |
| connect() | 透過 CDP 連接到 Chrome / Edge | 使用 `connectOverCDP()`，連接後記錄所有頁面到 log，並記錄實際瀏覽器版本 |
| disconnect() | 斷開與使用者瀏覽器的連接 | 只做 `this.browser = null`，NEVER 呼叫 `browser.close()` |
| requestShutdown() | 設定優雅關閉旗標 | 設定 isShuttingDown = true，讓迴圈在下一輪停止 |
| collectAll() | 自動模式完整蒐集 | 依設定檔的 pages 列表逐一導航與蒐集 |
| collectInteractive() | 互動模式蒐集 | 採用「頁面蒐集 → 流程錄製」兩階段選單，最多 100 頁，正常流程不需要用 Ctrl+C 切換階段；整次 `.\collect.ps1` 的輸出集中於同一個 `materials\<YYYYMMDDhhmmss_錄製名稱>\` 任務子資料夾 |
| collectSnapshot() | 快照模式 | 快速擷取當前頁面 |
| getActivePage() | 取得使用者可見的活躍頁面 | 自動過濾 Chrome / Edge 內部頁面 |
| resolvePageUrl(page) | 解析頁面真實 URL | 處理 CDP 預存頁面 page.url() === '' 的情況 |
| resolvePageTitle(page) | 解析頁面標題 | 先嘗試 page.title()，超時降級為 CDPSession |
| saveMetadata() | 儲存 metadata.json | 在 finally 區塊中確保執行；輸出到當次任務子資料夾根目錄 |
| generateSummaryReport() | 產生 summary-report.md | 在 finally 區塊中確保執行；輸出到當次任務子資料夾根目錄 |
...
| pre-commit-scan.ps1 | scripts/pre-commit-scan.ps1 | 掃描 `materials\<任務子資料夾>\recordings\*.ts` 中的密碼、token、secret 模式 |
...
1. **Given** 使用者選擇的 Chrome 或 Edge 已以 Debug 模式啟動並登入內部網站，**When** 執行 `.\collect.ps1`（若使用 Edge，需加上 `--browser edge`）並選擇互動模式，**Then** 系統連接到該瀏覽器、為本次任務準備 `materials\<YYYYMMDDhhmmss_錄製名稱>\` 任務子資料夾，並顯示兩階段工作說明與當前頁面資訊
2. **Given** 系統已連接到使用者選擇的瀏覽器，**When** 使用者輸入頁面名稱和描述，**Then** 系統自動擷取 ARIA 快照和截圖並儲存到本次任務子資料夾中的 `aria-snapshots\` 與 `screenshots\`
3. **Given** 已蒐集一個頁面，**When** 使用者透過數字選單選擇下一步，**Then** 系統能明確切換到「繼續蒐集 / 進入錄製 / 結束」其中之一
4. **Given** 使用者要從頁面蒐集切到流程錄製，**When** 使用者在選單中選擇切換階段，**Then** 系統不需要重新啟動也不需要依賴 Ctrl+C
5. **Given** 使用者選擇結束蒐集，**When** 使用者在選單中選擇結束，**Then** 系統在同一任務子資料夾根目錄儲存 metadata.json 和 summary-report.md，並安全斷開連接
...
1. **Given** 在互動模式中蒐集了頁面素材，**When** 使用者切換到錄製階段並選擇開始新的錄製，**Then** 系統提示輸入錄製名稱和起始 URL，並以該錄製名稱完成本次 `materials\<YYYYMMDDhhmmss_錄製名稱>\` 命名
2. **Given** 使用者確認開始錄製，**When** 系統啟動 Codegen，**Then** 開啟新的瀏覽器視窗讓使用者操作
3. **Given** Codegen 正在錄製，**When** 使用者關閉錄製視窗，**Then** 系統將錄製結果儲存到本次任務子資料夾的 `recordings\` 目錄
...
6. **Given** 錄製完成，**When** 使用者回到 CLI，**Then** 系統提供清楚的後續選單（重新錄製 / 再錄另一個 / 返回頁面蒐集 / 結束）
...
**驗收情境**：
1. **Given** 使用者選擇的 Chrome 或 Edge 已以 Debug 模式啟動並瀏覽目標頁面，**When** 執行 `npm run collect:snapshot`（或 `./collect.ps1 --snapshot`；使用 Edge 時加上 `--browser edge`），**Then** 系統自動擷取當前頁面的 ARIA 快照和截圖
2. **Given** 頁面包含 iframe，**Then** 系統遞迴擷取所有 iframe 的 ARIA 快照（最深 3 層）

### User Story 4 - 自動模式批次蒐集 (Priority: P2)

**驗收情境**：
1. **Given** `collect-materials-config.json` 已定義目標頁面列表，**When** 執行 `npm run collect:auto`（若使用 Edge，可在設定檔中把 `browser` 設為 `edge`，或改用 `./collect.ps1 --browser edge --auto`），**Then** 系統依序導航到每個頁面並蒐集素材
2. **Given** 某個頁面導航失敗，**Then** 記錄錯誤到 metadata.errors 並繼續處理下一個頁面

### User Story 5 - 瀏覽器 Debug 模式啟動 (Priority: P1)

**驗收情境**：
1. **Given** Google Chrome 或 Microsoft Edge 已安裝，**When** 執行對應的 `launch-chrome.ps1` / `launch-edge.ps1`，**Then** 啟動使用獨立 profile 的瀏覽器並開啟 CDP 端口 9222
2. **Given** 端口 9222 已被一般 Chrome / Edge 佔用，**Then** 偵測到非 debug 瀏覽器並提示使用者處理
3. **Given** Debug Chrome / Edge 已在運行，**Then** 偵測到已運行的 debug 瀏覽器並顯示下一步指引

### 邊緣案例

| 案例 | 處理方式 |
|------|---------|
| 瀏覽器內部頁面干擾 | `getActivePage()` 過濾 `chrome://`、`edge://` 等 URL |
| 端口被非 debug 瀏覽器占用 | `launch-chrome.ps1` / `launch-edge.ps1` 透過程序命令列檢查 |
| 全頁截圖失敗 | 自動降級為視窗截圖 |
| iframe 超過深度限制 | iframeDepth 限制遞迴（預設 3，最大 10） |
| Codegen 在 Windows 上 spawn 失敗 | 使用 cmd.exe 包裝 + windowsVerbatimArguments: true |
| 使用者 Ctrl+C 中斷 | 視為緊急中止；requestShutdown() → finally 區塊儲存 metadata |
...
| 編號 | 標準 |
|------|------|
| SC-001 | 使用者執行 .\install.ps1（實際轉呼叫 .\setup.ps1）即可完成環境安裝或檢查，無需手動輸入 npm |
| SC-002 | 使用者執行 `.\launch-chrome.ps1`（或 Edge 時 `.\launch-edge.ps1`）即可啟動對應 Debug 模式，包含端口衝突處理 |
...
| SC-006 | Chrome / Edge 內部頁面被正確過濾，不影響素材蒐集 |
...
從第一性原理推導：
├── 「無 AI」→ 需要 RPA-Cowork 離線蒐集 → 需要本地 Playwright
...
├── 前提：Codegen 使用獨立瀏覽器（非 CDP 連接的使用者瀏覽器）
├── 自動解析 URL 後導航 → session 不共享 → 擷取到錯誤頁面
├── 先在已登入的 Chrome / Edge 中擷取 ARIA → 保證正確狀態
...
- 穩定性：不得強制關閉使用者互動的瀏覽器（遵循 `connectOverCDP` 原則）；對 `chrome://`、`edge://` 等內部頁面進行過濾。
- 日誌：採 JSON Lines（每行一個 JSON 物件）為主，關鍵欄位：ts（ISO8601+08:00）、level、event、ctx（小型物件）。
- 時區：所有 timestamp 與 `.\collect.ps1` 任務子資料夾命名必須使用 Asia/Taipei（UTC+8）。
...
- 調試資料清單（必須收集並上傳 / 保留於測試提交包）
  - ARIA snapshot：materials\<任務子資料夾>\aria-snapshots\{pageSlug}_{ts}.json（或 $out\aria\…）
  - Screenshot：materials\<任務子資料夾>\screenshots\{pageSlug}_{ts}.png（或 $out\screenshots\…）
  ...
  - 若使用錄製（materials 根目錄下的任務子資料夾），請將新錄製放至 materials\<任務子資料夾>\recordings\ 並註明對應測試案例。
...
- 套件安裝（CI job）：
  - npm ci --silent
- 靜態檢查與 pre-commit：
  - 在 CI pipeline 中加入 pre-commit secret scan（例如 trufflehog 或 repo 的 pre-commit hooks），以及檢查 `materials\<任務子資料夾>\recordings\` 的錄製不含明文敏感資料。
- E2E 驗證（代表性驗證）：

...
console.log('━━ TEST 8: protect-env.mjs CLI 整合測試 ━━━━━━━━━━━━');
{
  ...
  try {
    ...
    // 執行加密
    const scriptPath = path.join(fakeScriptsDir, 'protect-env.mjs');
    const encResult = execFileSync(
    ...
    const statusResult = execFileSync(
      process.execPath, [scriptPath, '--status'],
      { cwd: tmpDir, encoding: 'utf8' }
    );
    assert(statusResult.includes('全部加密'), 'CLI --status 顯示全部加密');

    // 執行解密
    const decResult = execFileSync(
      process.execPath, [scriptPath, '--decrypt'],
      { cwd: tmpDir, encoding: 'utf8' }
    );
    ...
    const rotateResult = execFileSync(
      process.execPath, [scriptPath, '--rotate-key'],
      { cwd: tmpDir, encoding: 'utf8' }
    );
...
// TEST 9: 帶引號值的 CLI Encrypt→Decrypt round-trip
console.log('');
console.log('━━ TEST 9: 帶引號值 CLI Encrypt→Decrypt 往返 ━━━━━━━━');
{
  ...
  try {
    ...
    // 加密
    execFileSync(process.execPath, [scriptPath], { cwd: tmpDir, encoding: 'utf8' });

    // 解密
    execFileSync(process.execPath, [scriptPath, '--decrypt'], { cwd: tmpDir, encoding: 'utf8' });

    const decEnv = fs.readFileSync(path.join(tmpDir, '.env'), 'utf8');

    // 驗證值語意正確（解密後帶空白的值應自動加引號保護）
    assert(decEnv.includes('plain_value'), '解密後 NORMAL 值正確');
    assert(decEnv.includes('padded secret'), '解密後 QUOTED_SPACED 值保留空白');
    assert(decEnv.includes('single quoted'), '解密後 SINGLE_QUOTED 值正確');

    // 再次加密→解密確認完全 round-trip
    execFileSync(process.execPath, [scriptPath], { cwd: tmpDir, encoding: 'utf8' });
    execFileSync(process.execPath, [scriptPath, '--decrypt'], { cwd: tmpDir, encoding: 'utf8' });
    const finalEnv = fs.readFileSync(path.join(tmpDir, '.env'), 'utf8');
    assert(finalEnv.includes('padded secret'), '二次 round-trip QUOTED_SPACED 值仍正確');
  } finally {
    fs.rmSync(tmpDir, { recursive: true, force: true });
  }
}
